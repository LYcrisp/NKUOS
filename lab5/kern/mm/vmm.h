...
#define le2vma(le, member)                  \
    to_struct((le), struct vma_struct, member) // 将线性链表元素转换为vma_struct结构体

#define VM_READ                 0x00000001 // 可读权限
#define VM_WRITE                0x00000002 // 可写权限
#define VM_EXEC                 0x00000004 // 可执行权限
#define VM_STACK                0x00000008 // 栈区标志

// 控制结构体，用于管理使用相同PDT的一组vma
struct mm_struct {
    list_entry_t mmap_list;        // 线性链表链接，按vma的起始地址排序
    ...
    lock_t mm_lock;                // 互斥锁，用于使用dup_mmap函数复制mm
};
